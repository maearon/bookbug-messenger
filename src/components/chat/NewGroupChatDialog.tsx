...
interface FriendResponse {
  friends: { _id: string; displayName: string; email: string; avatar?: string }[];
}
...
//   const setActiveConversation = useChatStore(state => state.setActiveConversation);
  const { loading, createConversation, setActiveConversation, messages, fetchMessages } = useChatStore();
  ...
  const fetchFriends = useCallback(
    async (q: string, forceMinDelay = false) => {
      ...
      try {
        const res: FriendResponse = await friendService.getFriendsDialog(q);
        let list =
          res.friends?.map((f) => ({
  ...
  const handleCreateGroup = async () => {
    ...
    //   const id = await chatService.createConversation(payload.type, payload.name, payload.memberIds);
